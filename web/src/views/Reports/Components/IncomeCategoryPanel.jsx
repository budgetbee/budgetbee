...
export default function IncomeCategoryPanel({ searchData }) {
    ...
    useEffect(() => {
        ...
        async function fetch() {
            setIsLoading(true);
            const fetched = await Api.getIncomeCategoriesBalance(searchData);
    ...
    return (
        <div className="flex flex-col gap-y-4 p-5 bg-gray-700 rounded-3xl h-full">
            <div className="flex flex-row justify-between items-center">
                <div className="flex flex-col">
                    <span className="text-lg font-bold text-white">
                        {parentCategory ? parentCategory : "Income by Category"}
                    </span>
                    {parentCategory && (
                        <button
                            onClick={() => setParentCategory(null)}
                            className="text-xs text-green-400 hover:text-green-300 text-left mt-0.5"
                        >
                            ← All categories
                        </button>
                    )}
            ...
            {isLoading ? (
                ...
                <p className="text-gray-400 text-sm py-6 text-center">No income for this period.</p>
            ) : (
                <div className="flex flex-col gap-y-5">
                    {/* Doughnut */}
                    <div className="h-52 w-52 mx-auto" id="pdf-chart-income">
